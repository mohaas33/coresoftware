...
int TpcCombinedRawDataUnpacker::InitRun(PHCompositeNode* topNode)
{
  ...
  int bins_sector_arr[13]={8, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 8 };
  double phi_sector = 0.51;
  int bins_sector = 16; // 192/12 - 12 pads per bin in R3 and 128/8 - 8 pads per bin in R2 and 5.9 pad per bin in R1
  double phi_bin_width = phi_sector/bins_sector;
  int iphi_bin=0;
  for (int isec=0; isec<13;isec++){
    for(int iphi=0;iphi<bins_sector_arr[isec]+1;iphi++){
      if(isec<2)phi_bins[iphi_bin]=-M_PI + iphi*phi_bin_width + M_PI/12*isec;
      if(isec>1)phi_bins[iphi_bin]=-M_PI + iphi*phi_bin_width + M_PI/12 + 2*M_PI/12*(isec-1);
      iphi_bin++;
    }
   ...
   for (int i=0;i<nz+1;i++){
      z_bins[i]=-120+i*240/nz;
   }
   // Make variable bin size including R and phi bins
   double r_bins_new[2*(r_bins_N + 1)+nphi];
   //double r_bins_new[r_bins_N + 1];
   for(int i=0; i< r_bins_N + 1;i++){
       r_bins_new[i]=-1*r_bins[r_bins_N-i];
       r_bins_new[i+r_bins_N+ nphi ]=r_bins[i];
   }
   for(int i=0; i< nphi;i++){
      if(i==nphi-1){
         r_bins_new[i+r_bins_N]=M_PI;
      }
      else{
       r_bins_new[i+r_bins_N]=phi_bins[i];
      }
       //std::cout<< r_bins_new[i+r_bins_N] << std::endl;
   }

    m_file = new TFile(outfile_name.c_str(), "RECREATE");
   m_h_phiRZADC_0 = new TH3F("m_h_phiRZADC_0","m_h_phiRZADC_0;#phi;R;Z;ADC", (2*r_bins_N + nphi ), r_bins_new,(2*r_bins_N + nphi ), r_bins_new, nz, z_bins);
   m_h_phiRZADC_1 = new TH3F("m_h_phiRZADC_1","m_h_phiRZADC_1;#phi;R;Z;ADC", (2*r_bins_N + nphi ), r_bins_new,(2*r_bins_N + nphi ), r_bins_new, nz, z_bins);
  }
  if (m_writeTree)
  {
    m_file = new TFile(outfile_name.c_str(), "RECREATE");
      ...
      for (uint16_t s = 0; s < sam; s++)
      {
        ...
        if ((float(adc) - hpedestal) > (hpedwidth * m_ped_sig_cut))
        {
          ...
          if (!hit)
          {
            hit = new TrkrHitv2();
            if(m_writeHist){
              float w_ADC = float(adc) - hpedestal + pedestal_offset;
              if(phi>M_PI) phi -= 2*M_PI;
              if(side==0)m_h_phiRZADC_0->Fill(phi, R, t, w_ADC);
              if(side==1)m_h_phiRZADC_1->Fill(phi, R, t, w_ADC);
            }            
            if (m_do_baseline_corr)
            {
...
int TpcCombinedRawDataUnpacker::End(PHCompositeNode* /*topNode*/)
{
  ...
  {
    m_file->cd();
    m_h_phiRZADC_0->Write();
    m_h_phiRZADC_1->Write();
    m_file->Close();
  }
  if (m_writeTree)
  {
